export default function(root, options) {
    ...
    var getClosed = function(operator) {
        ...
        var id        = operator.getAttribute(attribute);
        var remember  = operator.getAttribute(options.rememberAttribute);

        if (remember && remember.length) {
            switch (remember) {
                case 'session':
                    return (sessionStorage.getItem(hash(window.location.hostname + id)) == 1)

                case 'local':
                    return (localStorage.getItem(hash(window.location.hostname + id)) == 1)
            }
        }
    ...
    var close = function(operator) {
        ...
        if (remember && remember.length) {
            switch (remember) {
                ...
                case 'local':
                    localStorage.setItem(hash(window.location.hostname + id), 1);
                    break;
            }
        }
